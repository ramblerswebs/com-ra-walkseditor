...
/**
 * Methods supporting a list of Ra_walkseditor records.
 *
 * @since  1.6
 */
class Ra_walkseditorModelPlaces extends \Joomla\CMS\MVC\Model\ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
            ...
            $query->from('`#__ra_walkseditor_places` AS a');
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_ra_walkseditor'))
		{
		...
		else
		{
            ...
            if (!empty($search))
            {
                ...
                else
                {
					...
					$query->where('( a.name LIKE ' . $search . '  OR  a.abbr LIKE ' . $search . '  OR  a.postcode LIKE ' . $search . '  OR  a.gridreference LIKE ' . $search . ' )');
                }
            }
            ...
            $orderCol  = $this->state->get('list.ordering', 'abbr');
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(Text::_("COM_RA_WALKSEDITOR_SEARCH_FILTER_DATE_FORMAT"), "warning");
