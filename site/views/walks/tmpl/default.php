...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post"
	...
	<?php echo JHtml::_('grid.sort', 'JPUBLISHED', 'a.state', $listDirn, $listOrder); ?>
			...
			<?php endif; ?>
				...
				<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_RA_WALKSEDITOR_WALKS_CATEGORY', 'a.category', $listDirn, $listOrder); ?>
				</th>
				<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_RA_WALKSEDITOR_WALKS_CONTENT', 'a.content', $listDirn, $listOrder); ?>
				</th>
				<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_RA_WALKSEDITOR_WALKS_STATUS', 'a.status', $listDirn, $listOrder); ?>
				</th>
	...
	<?php endif; ?>
				...
				<?php if (isset($item->checked_out) && $item->checked_out) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->uEditor, $item->checked_out_time, 'walks.', $canCheckin); ?>
				<?php endif; ?>
				<a href="<?php echo JRoute::_('index.php?option=com_ra_walkseditor&view=walk&id='.(int) $item->id . '&catid=' . (int) $item->category); ?>">
				<?php echo $this->escape($item->status); ?></a>
					...
					<td class="center">
						<?php if ($canEdit): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_ra_walkseditor&task=walk.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button"><i class="icon-edit" ></i></a>
						<?php endif; ?>
						<?php if ($canDelete): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_ra_walkseditor&task=walkform.remove&id=' . $item->id, false, 2); ?>" class="btn btn-mini delete-button" type="button"><i class="icon-trash" ></i></a>
						<?php endif; ?>
					</td>
				<?php endif; ?>
